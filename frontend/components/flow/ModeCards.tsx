...
>(function ModeCards({ modes, mode, onSelect }, heading) {
  return (
    <FieldSet className="gap-0">
      ...
      <RadioGroup value={mode ?? ""} onValueChange={(value) => onSelect(value as InputMode)} className="gap-3">
        {modes.map((value) => {
          ...
          return (
            <FieldLabel key={value} htmlFor={id} className="transition-colors duration-150">
              <Field orientation="horizontal" className="min-h-11 gap-4 has-[>[data-slot=field-content]]:items-center">
                ...
                {/* The card shows keyboard focus; the circle does not need a ring of its own. */}
                <RadioGroupItem value={value} id={id} className="focus-visible:ring-0 focus-visible:ring-offset-0" />
              </Field>
            </FieldLabel>
          );
        })}
